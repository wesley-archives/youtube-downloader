...
class YoutubeMp4Downloader:
    ...
    def downloadFromUrl(self, url):
        """Download video from a YouTube video using its URL."""
    ...
    def downloadFromFileByTitle(self, file):
        """Read a file containing titles (one per line), search YouTube, and download the video."""
    ...
    def downloadFromFileByUrl(self, file):
        """Read a file containing URLs (one per line) and download the video."""
        with open(file, 'r') as f:
            for line in f:
                url = line.strip()
                if url:
